 ...
 *
 * @responseField sourceDeinterlaceMode | Number | Current deinterlace mode
 *
...
RequestResult RequestHandler::GetSourceDeinterlaceMode(const Request &request)
{
	...
	json responseData;
	responseData["sourceDeinterlaceMode"] = obs_source_get_deinterlace_mode(source);
 ...
 *
 * @requestField sourceName          	| String | Name of the source
 * @requestField sourceDeinterlaceMode 	| Number | New deinterlace mode
 *
...
RequestResult RequestHandler::GetSourceDeinterlaceFieldOrder(const Request &request)
{
	...
	json responseData;
	responseData["sourceDeinterlaceFieldOrder"] = obs_source_get_deinterlace_field_order(source);
...
RequestResult RequestHandler::SetSourceDeinterlaceFieldOrder(const Request &request)
{
	...
	std::string comment;
	OBSSourceAutoRelease source = request.ValidateSource("sourceName", statusCode, comment);
	if (!source || !request.ValidateNumber("sourceDeinterlaceFieldOrder", statusCode, comment, true))
		return RequestResult::Error(statusCode, comment);
